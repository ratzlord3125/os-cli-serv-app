...
int main() {
    ...
    while(!satisfied) {
        ...
        while(1) {
            ...
            if(!busy_server) {
                ...
                connect_data->reg_req_flag = 0; 
                while(connect_data->reg_req_flag == 0); 
                
                if(connect_data->response_code) {
                    satisfied = 0; 
                    printf("UID already in use\n"); 
                }
                else {
                    satisfied = 1; 
    ...
    while(1) {
        if(data->reg_req_flag == 0) 
            continue; 
        
        int sc;
        
        if(data->times_serviced == 0) { // for first time service
            action_menu();
        ...
        else if(loop_state == 3)
        {
            ...
            munmap(channel_name,sizeof(struct client_data));
